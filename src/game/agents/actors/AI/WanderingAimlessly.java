...
public class WanderingAimlessly implements AI {
	...
	private boolean doAction(Actor actor, GameBoard gameBoard, Action action) {
		...
		switch (action) {
		case UP:
			if (gameBoard.getRoom(actor.getRoomNumber()).isTileAtCoordsPassable(actor.getCoords()[0], actor.getCoords()[1] - 1)) {
				actor.getCoords()[1]--;
				isSuccessful = true;
			}
			break;
		...
		case LEFT:
			...
			if (gameBoard.getRoom(actor.getRoomNumber()).isTileAtCoordsPassable(actor.getCoords()[0] - 1, actor.getCoords()[1])) {
				actor.getCoords()[0]--;
				isSuccessful = true;
			}
			break;
